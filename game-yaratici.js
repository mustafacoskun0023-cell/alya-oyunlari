...
window.GameMuzik = (function () {
  // Do-Re-Mi... 8 renkli tuş. Önce serbest çal, sonra ritmi tekrarla.
  const NOTA = [
    { ad: 'Do', hz: 261.63, hex: '#FF4757' },
    { ad: 'Re', hz: 293.66, hex: '#FF8A00' },
    { ad: 'Mi', hz: 329.63, hex: '#FFC800' },
    { ad: 'Fa', hz: 349.23, hex: '#22C55E' },
    { ad: 'Sol', hz: 392.00, hex: '#12C2C2' },
    { ad: 'La', hz: 440.00, hex: '#2E86FF' },
    { ad: 'Si', hz: 493.88, hex: '#8B5CF6' },
    { ad: 'Do²', hz: 523.25, hex: '#FF5FA2' }
  ];
  const TURLAR = [2, 3, 3, 4, 4];  // her turda kaç nota tekrarlanacak

  let ctx = null, tur = 0, hata = 0, dizi = [], beklenen = 0, kilit = false, ac = null;

  function ses(hz, sure) {
    try {
      ...
      g.gain.exponentialRampToValueAtTime(0.0001, ac.currentTime + (sure || 0.45));
      o.connect(g); g.connect(ac.destination);
      o.start(); o.stop(ac.currentTime + (sure || 0.45) + 0.05);
    } catch (e) {}
  }

  function mount(c) {
    ctx = c; tur = 0; hata = 0;
    ctx.say({ id: 'sys-oyun-muzik', text: 'Melodiyi dinle ve aynısını çal!' });
    ctx.duck(2600);
    setTimeout(kur, 2700);
  }
  ...
  function kur() {
    ctx.prompt.innerHTML = `
      <div class="prompt-side">
        <div class="prompt-q big">Dinle ve aynısını çal!</div>
        <div class="ritim-noktalar" id="ritimN"></div>
      </div>`;
    ctx.options.className = 'options-area piyano';
    ctx.options.style.gridTemplateColumns = `repeat(${innerHeight > innerWidth ? 4 : 8},1fr)`;
    ctx.options.innerHTML = '';
    NOTA.forEach((n, i) => {
      const b = document.createElement('button');
      b.className = 'tus';
      b.style.background = n.hex;
      b.innerHTML = `<span class="tus-ad">${n.ad}</span>`;
      b.dataset.i = i;
      b.addEventListener('click', () => bas(i, b));
      ctx.options.appendChild(b);
    });
    ctx.setProgress(tur, TURLAR.length);
    yeniDizi();
  }

  function yeniDizi() {
    const n = TURLAR[tur];
    dizi = Array.from({ length: n }, () => Math.floor(Math.random() * NOTA.length));
    beklenen = 0; kilit = true;
    noktalariCiz();
    ctx.duck(n * 700 + 900);
    setTimeout(() => diziCal(0), 700);
  }

  function diziCal(k) {
    if (k >= dizi.length) { kilit = false; return; }
    const i = dizi[k];
    const b = ctx.options.querySelector(`.tus[data-i="${i}"]`);
    if (b) { b.classList.add('calan'); setTimeout(() => b.classList.remove('calan'), 380); }
    ses(NOTA[i].hz);
    setTimeout(() => diziCal(k + 1), 620);
  }

  function noktalariCiz() {
    const el = ctx.prompt.querySelector('#ritimN');
    if (el) el.innerHTML = dizi.map((_, i) =>
      `<span class="rn ${i < beklenen ? 'dolu' : ''}"></span>`).join('');
  }

  function bas(i, b) {
    ses(NOTA[i].hz);
    b.classList.add('calan'); setTimeout(() => b.classList.remove('calan'), 220);
    if (kilit) return;
    if (i === dizi[beklenen]) {
      beklenen++;
      noktalariCiz();
      if (beklenen >= dizi.length) {
        ...
        if (tur >= TURLAR.length) {
          ...
          ctx.options.className = 'options-area';
          setTimeout(() => ctx.finish(yildiz, 'Bütün melodileri çaldın!'), 900);
        } else {
          Snd.sfx.star();
          setTimeout(yeniDizi, 1300);
        }
      }
    } else {
      hata++;
      ...
      Snd.sfx.wrong(); ctx.oops(); ctx.duck(1600);
      ctx.say({ id: 'sys-muzik-tekrar', text: 'Tekrar dinleyelim!' });
      setTimeout(() => diziCal(0), 1500);
    }
  }

  return {
    id: 'muzik', title: 'Müzik<br>ve Ritim', emoji: '🥁', mode: 'custom',
    intro: { id: 'sys-oyun-muzik', text: 'Melodiyi dinle ve aynısını çal!' },
    mount
  };
})();
